...
- **カラーオフセット修正**: 認識が不正確な場合（例：`RGB: (x, y, z)` 値が想定と異なる）、**演奏開始直後に一時停止**し（下のスクリーンショット参照）、ノーツが表示されているか判定線上にあることを確認してください。その後、「オフセット検出」をクリックします。ソフトウェアが自動的にオフセットを計算し、保存を促します。
