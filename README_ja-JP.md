...
- 連続タップまたはパフォーマンス問題: システムスケーリングを 100% に設定してください（2K ディスプレイでは 125% が動作する場合があります）。 マルチモニター環境では、ゲームウィンドウを必ずメインモニターに配置してください。また、座標認識の精度を保つため、ゲームウィンドウが他のウィンドウに隠れないようにしてください。「オフセット検出」を使用して、認識位置が正しくないかどうかをテストできます。**カラーオフセット修正**: 認識が不正確な場合（例：`RGB: (x, y, z)` 値が想定と異なる）、**演奏開始直後に一時停止**し（下のスクリーンショット参照）、ノーツが表示されているか判定線上にあることを確認してください。その後、「オフセット検出」をクリックします。ソフトウェアが自動的にオフセットを計算し、保存を促します。
