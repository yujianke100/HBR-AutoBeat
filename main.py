...
def deactivate_window(window_title="HeavenBurnsRed"):
    global window  # 声明为全局变量，以便在 on_press 中访问  # noqa: F824
...
class TransparentWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        client_left, client_top, client_width, client_height, y_value, min_x, max_x = (
            init("HeavenBurnsRed", test_flag=False)
        )
    ...
    def toggleHelp(self):
        ...
        QMessageBox.information(
            self, "Help", self.language_texts[self.language]["help"]
        )

    def initUI(self):
        global hold_th  # noqa: F824
        global LOCAL_VERSION  # noqa: F824
        # 设置窗口标志
        self.setWindowFlags(
